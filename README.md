...
* Quickly curate: add games manually with the inline form (just a title is required), remove entries using the × action, or click **Change match** in the detail view to browse IGDB’s top five alternatives and swap metadata with a single tap. A small browser cache stores the last rendered list so reopenings feel instant; use the “Clear cache” button if you want a fresh fetch.
